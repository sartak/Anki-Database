...
sub each_card {
    ...
    my $sth = $self->prepare('
        SELECT cards.id, cards.type, notes.flds, notes.id, notes.mid, cards.ord, notes.tags
            FROM cards
            JOIN notes ON cards.nid = notes.id
    ;');
    $sth->execute;

    while (my ($card_id, $type, $fields, $note_id, $model_id, $ordinal, $tags) = $sth->fetchrow_array) {
